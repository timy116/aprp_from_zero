...
class MonitorProfile(Model):
    ...
    def __str__(self):
        return str(
            f'product: {self.product.name}, watchlist: {self.watchlist.name}, price: {self.price}'
        )
    ...
    def sibling(self):
        return MonitorProfile.objects.exclude(id=self.id).filter(
            type=self.type, product=self.product, watchlist=self.watchlist
        )
